...
function wristband(ctx, cx, cy, w, h, angle, { name, college, id }) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(angle);
  const x = -w / 2, y = -h / 2;
...
function eventBlock(ctx, st, x, y, w, h, pick) {
  const plateW = Math.round(h * 4 / 3);
  if (pick === 'all') {
    const cw = plateW / 3, ch = h / 2;
    EVENTS.forEach((ev, i) => {
      plate(ctx, ev, x + (i % 3) * cw + 3, y + Math.floor(i / 3) * ch + 3, cw - 6, ch - 6, ev.plateStock || 'navy', false);
    });
  } else {
  ...
  const name = pick === 'all' ? 'All six events' : EVENTS[pick].name;
  ...
  const kind = pick === 'all' ? 'Two days · one pulse' : `${EVENTS[pick].kind} · ${EVENTS[pick].day.split(' · ')[0]}`;
  T(ctx, kind, tx, y + h - 8, mono(st.accent, 20, { weight: 600 }));
}

export function drawCard(canvas, { name, college, pick, format, heart, texture }) {
  ...
  if (format === 'story') {
    metaRow(ctx, st, 250, ['Nº IGN—26', 'Attendee', '09—10.10.26']);
    ...
    wristband(ctx, W / 2, 1010, W - 60, 250, -0.045, { name: nm, college: college.trim(), id });
    eventBlock(ctx, st, G, 1230, W - 2 * G, 300, pick);
    ctx.fillStyle = st.ink; ctx.fillRect(G, 1590, W - 2 * G, 2);
    T(ctx, 'Fri 09 & Sat 10 Oct 2026 · KPRIET, Coimbatore', W / 2, 1644, mono(st.ink2, 22, { align: 'center' }));
    T(ctx, SITE, W / 2, 1700, mono(st.accent, 24, { align: 'center', weight: 600, upper: false }));
    strip(ctx, st, 0, W, 1810, 60, { dot: W - 170 });
  } else {
    metaRow(ctx, st, 86, ['Nº IGN—26', 'Attendee', '09—10.10.26']);
    ...
    wristband(ctx, W / 2, 760, W - 60, 230, -0.045, { name: nm, college: college.trim(), id });
    eventBlock(ctx, st, G, 948, W - 2 * G, 230, pick);
    ctx.fillStyle = st.ink; ctx.fillRect(G, H - 92, W - 2 * G, 2);
    T(ctx, '09—10 Oct 2026 · KPRIET', G, H - 50, mono(st.ink, 21));
